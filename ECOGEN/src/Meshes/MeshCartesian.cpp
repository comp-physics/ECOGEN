...
void MeshCartesian::meshStretching() //KS//FP To improve at some point...
{
...
void MeshCartesian::initializeGeometrieParallele2(TypeMeshContainer<Cell *> &cells, TypeMeshContainer<CellInterface *> &cellInterfaces, std::string ordreCalcul)
{
  int ix, iy, iz;
  int compteMaillesParallele(0);
  ...
  m_numberCellsTotal = m_numberCellsCalcul; //KS//BD// Update this after ghost cells and after balancing

  m_numberFacesTotal = 0; //KS//BD// To update in general and after balancing
  ...
  //Faces and ghost cells
  //---------------------

  createNeighbours(cells, ordreCalcul);
...
void MeshCartesian::createNeighbours(TypeMeshContainer<Cell *> &cells, std::string ordreCalcul)
{
  // Cell* dummy ;
  // if (ordreCalcul == "FIRSTORDER") { dummy = new Cell; }
  // else { dummy = new CellO2; }

  // for(unsigned int i = 0; i < cells.size(); ++i)
  // {

  //   std::array<decomposition::Key<3>,6> neighbours;
  //   for(int d = 0; d < 3; d++)
  //   {
  //    neighbours[2*d]   = decomposition::Key<3>(cells[i]->getElement()->getKey().coordinate()[d]-1, base_level);
  //    neighbours[2*d+1] = decomposition::Key<3>(cells[i]->getElement()->getKey().coordinate()[d]+1, base_level);
  //    //check if noughbour is out of physical bound => create boundary face
  //   }

  //   for(auto& e : neighbours) //Do this loop just over the keys within the bound
  //   {
  //         dummy->getElement()->setKey(e);
  //         auto it = std::lower_bound(cells.begin(), cells.end(), &dummy, 
  //                                   [](const auto& _k0, const auto& _k1){ return _k0->getElement()->getKey() < _k1->getElement()->getKey(); });
  //         if(it != cells.end())
  //         {
  //           //create face //internal face
  //         }
  //         else
  //         {
  //           //create ghost cell and face
...
void MeshCartesian::recupereCoord(std::vector<Cell *> *cellsLvl, std::vector<double> &jeuDonnees, Axe axe) const
{
  switch (axe) {
  case X:
...
void MeshCartesian::recupereDonnees(std::vector<Cell *> *cellsLvl, std::vector<double> &jeuDonnees, const int var, int phase) const
{
  jeuDonnees.clear();
  int numCell;
  for (int k = 0; k < m_numberCellsZ; k++) {
...
void MeshCartesian::setDataSet(std::vector<double> &jeuDonnees, std::vector<Cell *> *cellsLvl, const int var, int phase) const
{
  int iterDataSet(0);
  int numCell;
  Coord vec;

...
int main(int argc, char* argv[])
{
  ...
  while (elementTestCase != NULL) {
    try {
      ...
      //if (rankCpu == 0) { cout << "wait launch test" << std::endl;  system("pause"); }
      //MPI_Barrier(MPI_COMM_WORLD);
      run->initialize(argc, argv);
      run->solver();
      //if (rankCpu == 0) { cout << "wait end test" << std::endl; system("pause"); }
      //MPI_Barrier(MPI_COMM_WORLD);
      run->finalize();
      //3) Removal of the test case
      delete run;
      //if (rankCpu == 0) { cout << "test finished" << std::endl; system("pause"); }
      //MPI_Barrier(MPI_COMM_WORLD);
    }
